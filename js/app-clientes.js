...
window.getAllClients = getAllClients; // Attach to the global window object
...
function renderClientTable() {
	...
	clients.forEach((client) => {
		...
		row.innerHTML = `
            ...
            <td class="px-4 py-3">${
							client.activo
								? '<div class="bg-green-100 text-green-800 px-2 py-1 text-center rounded">Activo</div>'
								: '<div class="bg-red-100 text-red-800 px-2 py-1 text-center rounded"><p>Inactivo</p></div>'
						}</td>
            <td class="px-4 py-3">
...
function confirmActivateClient(id, nombre) {
	document.getElementById(
		'activar-confirm-message'
	).textContent = `¿Estás seguro que deseas activar al cliente "${nombre}"?`;
